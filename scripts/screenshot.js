...
if (args.length === 0) {
  console.log('Usage: node scripts/screenshot.js <screen-key> [screen-key2,screen-key3...]');
  console.log('\nAvailable screens:');
        ...
        try {
          ...
          await page.evaluate((selector) => {
            const elements = Array.from(document.querySelectorAll('*')).filter(el => 
              el.textContent?.includes(selector.replace('button:has-text("', '').replace('")', ''))
            );
...
async function takeScreenshots() {
  ...
  try {
    for (const screenKey of screenKeys) {
      ...
      console.log(`📸 Taking screenshot: ${screenKey} (${screenConfig.description})`);
      ...
      if (screenConfig.waitFor) {
        try {
          await page.waitForSelector(screenConfig.waitFor, { timeout: CONFIG.defaultTimeout });
        } catch (e) {
          console.log(`⚠️  Warning: Expected element not found for ${screenKey}: ${screenConfig.waitFor}`);
        }
      }
